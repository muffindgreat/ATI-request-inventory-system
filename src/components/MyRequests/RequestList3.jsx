    ...
    return (
      <Box sx={{ mt: 2, width: "100%", overflow: "hidden" }}>
        {items.length > 0 ? (
          ...
          <Typography sx={{ textAlign: "center", mt: 3, color: "gray" }}>No Requests Found</Typography>
        )}
      </Box>
    );
  }
...
{/* Material Info */}
<Box sx={{ flexGrow: 1 }}>
  <Typography variant="body1" sx={{ fontWeight: "bold", mb: 0.5,     display: "-webkit-box",
  ...
  {firstMaterial.bannerPrograms?.length > 0 && (
    ...
    >
      {firstMaterial.bannerPrograms.map((program, i) => (
        <Chip key={i} label={program} color="primary" size="small" sx={{ m: 0 }} /> // ✅ Remove extra margins
      ))}
    </Stack>
  )}
</Box>
        ...
        {materials.map((material, index) => (
          <Box key={index}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", my: 1, p: 2 }}>
              ...
              <Typography variant="body2" color="textSecondary">x{material.quantity}</Typography>
            </Box>
          </Box>
        ))}
      </>
